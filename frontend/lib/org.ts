...
export async function getUserOrgId(userId: string, email?: string): Promise<string | null> {
  ...
  const allMembers = [
    ...(nextAuthMembers as unknown as Record<string, unknown>[]),
    ...(mongooseMembers as unknown as Record<string, unknown>[]),
  ];
...
export async function getOrgDetails(orgId: string): Promise<Record<string, unknown> | null> {
  ...
  const queries: Promise<any>[] = [db.collection(collections.organizations).findOne({ id: orgId })];
  if (oid) {
