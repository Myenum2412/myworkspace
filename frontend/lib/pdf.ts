...
export async function generateInvoicePDF(baseInvoice: Invoice, preview = false) {
  ...
  doc.setFont("helvetica", "bold");
  doc.text(
    `: ${new Date(invoice.dueDate || invoice.createdAt).toLocaleDateString()}`,
    leftValX,
    r1Y + 12,
  );
  doc.setFont("helvetica", "normal");
  ...
  doc.text(
    (client.name || invoice.customerName || "Customer Name").toUpperCase(),
    mX + 2,
    gridY + 5,
  );
  doc.setFont("helvetica", "normal");
  ...
  const subTotal =
    invoice.subTotal ||
    items.reduce((acc: number, i: any) => acc + (i.quantity || 1) * (i.rate || 0), 0);
  ...
  doc.text(subTotal.toFixed(2), valX, rightY, { align: "right" });
  rightY += 6;
  ...
  doc.text(cgstTotal.toFixed(2), valX, rightY, { align: "right" });
  rightY += 6;

  doc.text(`SGST9 (${halfTax}%)`, rightX + 2, rightY);
  doc.text(sgstTotal.toFixed(2), valX, rightY, { align: "right" });
  rightY += 6;
