...
function getFileCategory(mimeType: string, fileName: string): FileCategory {
  ...
  if (
    ["jpg", "jpeg", "png", "gif", "svg", "webp", "bmp", "tiff", "tif", "ico", "avif"].includes(ext)
  )
    return "image";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-screen-xl w-full min-w-[95vw] max-h-[95vh] h-[90vh] p-0 flex flex-col">
        ...
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col lg:flex-row">
          <div
            ref={viewerRef}
            className="flex-1 min-h-0 min-w-0 flex items-center justify-center bg-muted/30 relative overflow-hidden"
          >
            {loading && (
            ...
            {!loading && !error && category === "pdf" && (
              <div className="w-full h-full" onKeyDown={handlePdfKeyDown} tabIndex={0}>
                <iframe
            ...
            {!loading && !error && category === "archive" && (
              <div className="flex flex-col items-center justify-center p-8 text-center gap-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Archive files cannot be previewed inline.
                </p>
          ...
          <div className="w-full lg:w-64 shrink-0 border-t lg:border-t-0 lg:border-l bg-card overflow-y-auto">
            <div className="p-4 space-y-3">
              ...
              {file.uploadedAt && (
                <MetadataRow
                  label="Uploaded"
                  value={new Date(file.uploadedAt).toLocaleDateString(undefined, {
                    year: "numeric",
                    month: "short",
                    day: "numeric",
                  })}
