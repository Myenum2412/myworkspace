...
import { Search, ArrowLeft, WifiOff, FileQuestion } from "lucide-react";
...
function Illustration(props: React.ComponentPropsWithoutRef<"svg">) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 362 145" className="size-48 sm:size-56" {...props}>
      <path
        fill="currentColor"
...
export function NotFoundContent({ variant }: NotFoundContentProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-4">
      <div className="relative text-center z-[1] pt-12">
        ...
        <h1 className="mt-4 text-balance text-4xl font-semibold tracking-tight text-primary sm:text-5xl">
          {isOffline ? "No internet connection" : "Page not found"}
        </h1>
        <p className="mt-4 text-pretty text-base font-medium text-muted-foreground sm:text-lg/8">
          {isOffline
        ...
        <div className="mt-10 flex flex-col sm:flex-row sm:items-center sm:justify-center gap-y-3 gap-x-6">
          ...
          {isOffline ? (
            <Button className="-order-1 sm:order-none" onClick={() => window.location.reload()}>
              Retry connection
            </Button>
          ) : (
