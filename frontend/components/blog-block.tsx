...
const posts = [
  {
    slug: "cut-api-latency-edge-caching",
  ...
  {
    slug: "token-system-survives-rebrand",
    category: "Design",
  ...
  {
    slug: "user-interviews-onboarding",
    category: "Product",
      ...
      <div className="mx-auto w-full max-w-5xl">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex flex-col gap-1.5">
            ...
            <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">From the Acme team</h2>
          </div>
          <Button variant="outline" size="sm" className="group gap-1.5 transition-colors" asChild>
            <Link href="/blog">
              All Articles
              <RiArrowRightLine className="transition-transform duration-200 group-hover:translate-x-0.5 size-[15px]" />
            </Link>
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
          {posts.map((post) => (
            <Link key={post.title} href={`/blog/${post.slug}`} className="group">
              <Card className="flex flex-col overflow-hidden border-border pt-0 transition-shadow duration-200 hover:shadow-md h-full">
