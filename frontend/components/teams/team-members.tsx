...
import type { OrgMember, TeamDetail, TeamMember } from "./team-types";
...
export function TeamMembers({
  ...
  tableView,
  memberPage = 0,
  memberRowsPerPage = 30,
  onMemberPageChange,
  onMemberRowsPerPageChange,
  onBack,
  onDelete,
  showAddMember,
  ...
  const allSelected =
    paginatedMembers.length > 0 && paginatedMembers.every((m) => selectedIds.includes(m.id));
    ...
    <>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {m.role === "team_lead" && (
                                <Badge className="bg-amber-500 text-white text-[10px] px-1.5 py-0">
                                  <CrownIcon className="size-3 mr-0.5 fill-white" /> Lead
                                </Badge>
                              )}
                            </div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      onMemberRowsPerPageChange?.(Number(value));
                    }}
                  >
            ...
            {team.members.length === 0 ? (
              ...
              <div className="space-y-2 p-4">
                {team.members.map((m) => (
                  <div
                    key={m.id}
                    className="flex items-center justify-between rounded-sm border p-3 cursor-pointer hover:bg-blue-50/50 transition-colors"
                    onClick={() => {
                      onViewMemberChange(m);
                      onViewMemberOpenChange(true);
                    }}
      ...
      {showAddMember && (
        <div
          ...
          onClick={() => onShowAddMemberChange(false)}
        >
          <div
            className="w-full max-w-md rounded-sm bg-background p-5 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            {availableMembers.length === 0 ? (
              ...
              <div className="space-y-1 max-h-64 overflow-y-auto">
                {availableMembers.map((m) => (
                  <button
                    key={m.userId}
                    className="w-full flex items-center gap-3 rounded-sm p-2 hover:bg-muted transition-colors text-left"
      ...
      <Dialog
        ...
        onOpenChange={(o) => {
          if (!o) {
            onViewMemberOpenChange(false);
            onViewMemberChange(null);
          }
        }}
      >
              ...
              <DialogHeader className="px-6 pt-6 pb-2 shrink-0">
                ...
                <DialogDescription>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="capitalize text-xs bg-muted rounded-2xl px-2.5 py-0.5">
                      {viewMember.role}
                    </span>
