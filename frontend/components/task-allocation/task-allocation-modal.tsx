  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/projects-list", { credentials: "include", signal })
        .then((r) => r.json())
        .catch(() => ({ data: [] })),
      fetch("/api/user/me", { credentials: "include", signal })
        .then((r) => r.json())
        .catch(() => ({})),
      fetch("/api/settings", { credentials: "include", signal })
        .then((r) => r.json())
        .catch(() => null),
      ...
      const mappedProjects = projectArr.map(
        (p: { id?: string; name?: string; client?: string }) => ({
          id: p.id || "",
          name: p.name || "",
          client: p.client || "",
        }),
      ...
      <div className="w-full h-full sm:relative sm:w-[95vw] sm:h-[95vh] sm:mx-auto sm:mt-[2.5vh] sm:rounded-sm bg-card flex flex-col overflow-hidden">
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Create and assign work to your team</p>
          </div>
                    ...
                    <SelectContent>
                      {selectedClient
                        ? projectList
                            ...
                            .map((p) => (
                              <SelectItem key={p.id} value={p.name} className="text-sm truncate">
                                {p.name}
                              </SelectItem>
                            ))
                        : projectList.map((p) => (
            ...
            <div className="md:col-span-3 flex flex-col min-h-0">
              <FormField label="Description" required className="flex-1 flex flex-col min-h-0">
                <div className="flex-1 flex flex-col min-h-0">
                  <BlogEditor value={description} onChange={setDescription} placeholder="" />
                </div>
              </FormField>
            </div>
          </div>
