...
function isOfficeFile(file: FileItem): boolean {
  const ext = getFileExtension(file.originalName);
  return (
    OFFICE_TYPES.includes(file.mimeType) ||
...
function isArchiveFile(file: FileItem): boolean {
  const ext = getFileExtension(file.originalName);
  return (
    ARCHIVE_TYPES.includes(file.mimeType) || ["zip", "rar", "7z", "tar", "gz", "bz2"].includes(ext)
  );
}
...
function isModelFile(file: FileItem): boolean {
  const ext = getFileExtension(file.originalName);
  return (
    MODEL_TYPES.includes(file.mimeType) || ["glb", "gltf", "obj", "fbx", "stl", "ply"].includes(ext)
  );
}
...
function isCADFile(file: FileItem): boolean {
  const ext = getFileExtension(file.originalName);
  return (
    CAD_TYPES.includes(file.mimeType) ||
    ["dwg", "dxf", "ifc", "dgn", "stp", "step", "igs", "iges"].includes(ext)
  );
}
        ...
        return (
          <div className="flex flex-col items-center justify-center h-full gap-4 text-muted-foreground p-8">
            ...
            <div className="text-center">
              ...
              <p className="text-xs">
                {file.mimeType || "Unknown type"} &middot; {formatSize(file.size)}
              </p>
            </div>
  ...
  if (showInfo) {
    return (
      <Tabs defaultValue="preview" className="flex flex-col h-full">
        ...
        <TabsContent
          value="preview"
          className="flex-1 min-h-0 p-0 m-0 data-[state=active]:flex flex-col"
        >
          {renderViewer()}
        </TabsContent>
        <TabsContent
          value="info"
          className="flex-1 min-h-0 overflow-y-auto m-0 data-[state=active]:block"
        >
