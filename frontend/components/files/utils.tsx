...
export function getFileIcon(mimeType: string, fileName?: string) {
  ...
  if (extCat === "image" || imageTypes.includes(mimeType))
    return <ImageIcon className="size-5 text-sky-500" />;
  if (extCat === "video" || videoTypes.includes(mimeType))
    return <VideoIcon className="size-5 text-purple-500" />;
  if (extCat === "audio" || audioTypes.includes(mimeType))
    return <MusicIcon className="size-5 text-emerald-500" />;
  if (mimeType === "application/pdf" || extCat === "pdf")
    return <FileTextIcon className="size-5 text-red-500" />;
  if (extCat === "spreadsheet" || spreadsheetTypes.includes(mimeType))
    return <TableIcon className="size-5 text-green-600" />;
  if (extCat === "presentation" || presentationTypes.includes(mimeType))
  ...
  if (extCat === "archive" || archiveTypes.includes(mimeType))
    return <ArchiveIcon className="size-5 text-amber-500" />;
  if (extCat === "code" || codeTypes.includes(mimeType))
    return <FileCodeIcon className="size-5 text-indigo-500" />;
  ...
  if (mimeType.includes("json") || mimeType.includes("xml"))
    return <FileJsonIcon className="size-5 text-yellow-500" />;
...
export function getFileTypeColor(mimeType: string, fileName?: string): string {
  ...
  if (extCat === "code" || mimeType.startsWith("text/"))
