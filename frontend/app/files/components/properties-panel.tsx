...
export function PropertiesPanel() {
  ...
  const { data: folder } = useQuery({
    ...
    queryFn: () =>
      propertiesTarget?.type === "folder" ? api.getFolder(propertiesTarget.id) : null,
        ...
        <SheetHeader>
          <div className="flex items-center gap-3">
            {isFile ? (
              ...
              <FolderIcon className="size-5 text-primary/60" />
            )}
            <div className="min-w-0">
              ...
              <SheetDescription>
                {isFile ? "File Properties" : "Folder Properties"}
              </SheetDescription>
            </div>
          </div>
        </SheetHeader>
          ...
          {isFile && file ? (
            <>
              ...
              <Field
                icon={HashIcon}
                label="Version"
                value={file.currentVersion ? `v${file.currentVersion}` : "v1"}
              />
              <Separator />
              <Field
                icon={UserIcon}
                label="Uploaded By"
                value={file.uploaderName || file.uploaderId}
              />
              <Field
                icon={CalendarIcon}
                label="Created"
              ...
              <Field
                icon={LockIcon}
                label="Locked"
                value={file.isLocked ? `Yes${file.lockedBy ? ` (by ${file.lockedBy})` : ""}` : "No"}
              />
              <Field
                icon={ShieldCheckIcon}
                label="Virus Scan"
                value={file.virusScanStatus || "pending"}
              />
              {file.description && (
                <>
                  <Separator />
            ...
            <>
              ...
              <Field
                icon={CalendarIcon}
                label="Created"
                value={folder.createdAt ? new Date(folder.createdAt).toLocaleString() : "—"}
              />
              {folder.description && (
