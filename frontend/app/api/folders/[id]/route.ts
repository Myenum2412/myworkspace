...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const member = await db
    ...
    .findOne({ userId: session.user.id, orgId: folder.orgId });
  if (!member) return NextResponse.json({ error: "Not authorized" }, { status: 403 });

  const children = await (await db.collection("folders").find({ parentId: id, deletedAt: null }))
    .sort({ name: 1 })
    .toArray();
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parent = folder.parentId
    ? await db.collection("folders").findOne({ id: folder.parentId })
    : null;
  const newPath = parent ? `${parent.path}/${name}` : `/${name}`;

  const existing = await db
    .collection("folders")
    .findOne({ orgId: folder.orgId, path: newPath, deletedAt: null, id: { $ne: id } });
  if (existing) return NextResponse.json({ error: "Folder name already exists" }, { status: 409 });

  const oldPath = folder.path;
  await db
    .collection("folders")
    .updateOne({ id }, { $set: { name, path: newPath, updatedAt: new Date() } });
      ...
      $set: {
        path: {
          $toString: {
            $concat: [
              newPath + "/",
              { $substrCP: ["$path", oldPath.length + 1, { $strLenCP: "$path" }] },
            ],
          },
        },
      },
