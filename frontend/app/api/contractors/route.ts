...
export async function POST(req: Request) {
  ...
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(body.emailAddress)))
    fields.emailAddress = "Invalid email format";
  if (!String(body.mainTrade || "").trim()) fields.mainTrade = "Main trade is required";
  if (!String(body.accountHolderName || "").trim())
    fields.accountHolderName = "Account holder name is required";
  ...
  const validContacts = emergencyContacts.filter(
    (c: any) => c?.name?.trim() && c?.phoneNumber?.trim(),
  );
  if (validContacts.length === 0)
    fields.emergencyContacts = "At least one emergency contact with name and phone is required";
  ...
  try {
    const existing = await db
      .collection(collections.contractors)
      .findOne({ orgId, emailAddress: String(body.emailAddress).toLowerCase().trim() });
    if (existing)
      return NextResponse.json(
        {
          error: "A contractor with this email already exists",
          fields: { emailAddress: "Email already in use" },
        },
        { status: 409 },
      );
