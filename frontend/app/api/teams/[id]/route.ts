...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const team = oid ? await db.collection(collections.teams).findOne({ _id: oid, orgId }) : await db.collection(collections.teams).findOne({ id, orgId });
  ...
  return NextResponse.json({ success: true, data: { id: teamIdStr, name: team.name, description: team.description || "", createdAt: team.createdAt, members: enrichedMembers } });
}
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await db.collection(collections.teams).updateOne(query, { $set: { name, description, updatedAt: new Date(), updatedBy: session.user.id } });
  if (result.matchedCount === 0) return NextResponse.json({ error: "Team not found" }, { status: 404 });
