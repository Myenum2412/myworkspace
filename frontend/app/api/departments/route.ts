...
export async function GET() {
  ...
  try {
    ...
    const departments = (deptData as any[]).map((d, i) => {
      ...
      return {
        name: d.name || "Unknown",
        head: headUser?.name || "Unassigned",
        headAvatar: headUser?.image || "",
        memberCount: d.memberCount || (members as any[]).length,
        openPositions: 0,
        budget: "$0",
    ...
    return NextResponse.json({ departments: [], totalMembers: 0, totalOpen: 0 });
  }
}
