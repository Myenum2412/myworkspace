...
export async function POST(req: Request) {
  ...
  if (!String(body.patientName || "").trim())
    return NextResponse.json(
      { error: "Patient name is required", success: false },
      { status: 422 },
    );
  if (!String(body.doctorId || "").trim())
    return NextResponse.json({ error: "Doctor is required", success: false }, { status: 422 });
  if (!String(body.appointmentDate || "").trim())
    return NextResponse.json(
      { error: "Appointment date is required", success: false },
      { status: 422 },
    );
  ...
  try {
    ...
    if (!doctorName && body.doctorId) {
      const doctor = (await db
        .collection(collections.doctors)
        .findOne({ id: String(body.doctorId), orgId })) as any;
