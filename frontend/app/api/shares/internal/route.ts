...
export async function GET(req: Request) {
  ...
  if (orgId) {
    const member = await db
      ...
      .findOne({ userId: session.user.id, orgId });
    if (!member) return NextResponse.json({ error: "Not authorized" }, { status: 403 });

    const filter: Record<string, unknown> = { orgId };
    if (fileId) filter.fileId = fileId;

    const shares = await (await db.collection(collections.fileShares).find(filter))
      .sort({ createdAt: -1 })
      .toArray();
    const fIds = [...new Set(shares.map((s) => s.fileId))];
    const files = await (
      await db.collection(collections.fileAttachments).find({ id: { $in: fIds }, deletedAt: null })
    ).toArray();
  ...
  if (userId) {
    const shares = await (
      await db.collection(collections.fileShares).find({
...
export async function POST(req: Request) {
  ...
  if (!fileId || !orgId)
    return NextResponse.json({ error: "fileId and orgId are required" }, { status: 400 });
  ...
  const file = await db
    ...
    .findOne({ id: fileId, deletedAt: null });
...
export async function DELETE(req: Request) {
  ...
  if (share.orgId) {
    const member = await db
      .collection(collections.orgMembers)
      .findOne({ userId: session.user.id, orgId: share.orgId });
    if (!member)
