...
export async function POST(req: Request) {
  ...
  if (!fileIds?.length || !tags?.length)
    return NextResponse.json({ error: "fileIds and tags are required" }, { status: 400 });
  ...
  if (action === "remove") {
    await db
      ...
      .updateMany({ id: { $in: fileIds } }, { $pullAll: { tags } });
  } else {
    await db
      .collection(collections.fileAttachments)
      .updateMany({ id: { $in: fileIds } }, { $addToSet: { tags: { $each: tags } } });
  }
