...
export async function GET() {
  ...
  try {
    ...
    const items = (raw as any[]).map((s) => ({
      ...
      itemCode: s.itemCode || "", productName: s.productName || s.name || "",
      category: s.category || "", brand: s.brand || "", unit: s.unit || "",
      openingStock: Number(s.openingStock ?? 0), stockIn: Number(s.stockIn ?? 0),
      stockOut: Number(s.stockOut ?? 0), availableStock: Number(s.availableStock ?? s.quantity ?? 0),
      reorderLevel: Number(s.reorderLevel ?? 0), purchasePrice: Number(s.purchasePrice ?? s.price ?? 0),
      sellingPrice: Number(s.sellingPrice ?? 0), supplier: s.supplier || "",
      warehouse: s.warehouse || "", status: s.status || "Active",
...
export async function POST(req: Request) {
  ...
  if (!String(body.productName || "").trim()) return NextResponse.json({ error: "Product name is required" }, { status: 422 });
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: { stock: { id, productName: doc.productName, status: doc.status } } }, { status: 201 });
  } catch (err) {
