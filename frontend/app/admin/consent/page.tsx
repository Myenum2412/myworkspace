...
export default function AdminConsentPage() {
  ...
  const fetchStats = async (range: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/consent/stats?from=${from.toISOString()}`, {
        credentials: "include",
      });
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Cookie consent analytics and compliance monitoring
          </p>
        </div>
        ...
        <>
          <Stats07
            items={[
              ...
              {
                name: "Analytics",
                value: stats.acceptanceRates.analytics,
                subtitle: "Analytics rate %",
              },
              {
                name: "Marketing",
                value: stats.acceptanceRates.marketing,
                subtitle: "Marketing rate %",
              },
              {
                name: "Functional",
                value: stats.acceptanceRates.functional,
                subtitle: "Functional rate %",
              },
            ]}
          />
