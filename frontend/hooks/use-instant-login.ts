...
export function useInstantLogin() {
  ...
  const instantLogin = useCallback(
    async (email: string, password: string) => {
      ...
      try {
        ...
        try {
          ...
          setState({
            loading: false,
            error: "Authentication service unavailable. Check your connection.",
            step: "idle",
          });
          return;
        }
        ...
        const [bootstrapData] = await Promise.all([fetchBootstrapData(true), updateSession()]);
