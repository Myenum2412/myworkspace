...
for (const l of logs) console.log(JSON.stringify({at:l.createdAt, orgId:l.orgId, userId:l.userId, action:l.action, desc:(l.description||"").slice(0,110)}));
...
console.log("STAFFS:");
for(const u of staffs) console.log(JSON.stringify({id:u.id,email:u.email,orgId:u.orgId,role:u.role,_id:u._id.toString()}));
console.log("MEMBER RECORDS for staff users:");
for(const u of staffs){ const ms = await db.collection("org_members").find({userId:u._id.toString()}).toArray(); console.log(u.email, "->", ms.map(m=>`{orgId:${m.orgId},role:${m.role}}`).join(", ")); }
await c.close();
