...
const users = await db.collection("users").find({}).project({ email: 1, name: 1, orgId: 1, role: 1, status: 1, isActive: 1 }).toArray();
let mismatchCount = 0;
for (const u of users) {
  ...
  if ((userOrg && memberOrg && userOrg !== memberOrg) || (!userOrg && memberOrg) || (userOrg && members.length === 0)) {
    mismatchCount++;
    log(`- ${u.email || u._id}: users.orgId=${userOrg || "<none>"} | orgmembers=${memberOrgs.join(",") || "<none>"} role=${u.role} active=${u.isActive} status=${u.status}`);
  }
}
...
try {
  const taskOrgs = await db.collection("tasks").aggregate([{ $group: { _id: "$orgId", n: { $sum: 1 } } }, { $sort: { n: -1 } }]).toArray();
  log(JSON.stringify(taskOrgs));
} catch (e) { log("tasks agg err", e.message); }
