...
function scanFile(filePath: string, category: AuditEntry["type"]): AuditEntry {
  ...
  for (const line of lines) {
    ...
    if (
      trimmed.includes("processEvent(") ||
      trimmed.includes("createNotification(") ||
      trimmed.includes("notifyTask.") ||
      trimmed.includes("notifyProject.") ||
      trimmed.includes("notifyFile.") ||
      trimmed.includes("notifyAuth.") ||
      trimmed.includes("notifyBilling.") ||
      trimmed.includes("notifyClient.") ||
      trimmed.includes("notifyHR.") ||
      trimmed.includes("notifyApproval.") ||
      trimmed.includes("notifyPermission.") ||
      trimmed.includes("notifyCommunication.") ||
      trimmed.includes("notifySecurity.") ||
      trimmed.includes("notifySystem.") ||
      trimmed.includes("notifyMultiUser.") ||
      trimmed.includes("broadcastNotification(") ||
    ...
    if (
      trimmed.includes("sendEmail(") ||
      trimmed.includes("sendMail(") ||
      trimmed.includes("sendWelcomeEmail(") ||
      trimmed.includes("sendPasswordResetEmail(") ||
      trimmed.includes("sendVerificationEmail(") ||
      trimmed.includes("sendOrganizationInviteEmail(") ||
      trimmed.includes("sendSignupOtpEmail(") ||
      trimmed.includes("sendPasswordDeliveredEmail(") ||
      trimmed.includes("sendTask") ||
      trimmed.includes("sendDailyDigest(") ||
      trimmed.includes("sendWeeklyDigest(") ||
      trimmed.includes("sendUnreadNotificationsReminder(") ||
  ...
  if (
    content.includes("Notification.create(") &&
    !content.includes("processEvent(") &&
    !content.includes("createNotification(")
  ) {
...
const modTotal =
  routeFiles.length +
  results.filter((r) => r.type === "service").length +
  results.filter((r) => r.type === "lib").length +
...
console.log("-".repeat(40));
console.log(
  `Routes with notifications:  ${routedWithNotifs.length}/${routeFiles.length} (${Math.round((routedWithNotifs.length / routeFiles.length) * 100)}%)`,
);
console.log(
  `Routes with emails:         ${routedWithEmail.length}/${routeFiles.length} (${Math.round((routedWithEmail.length / routeFiles.length) * 100)}%)`,
);
console.log(
  `Routes with audit logs:     ${routedWithAudit.length}/${routeFiles.length} (${Math.round((routedWithAudit.length / routeFiles.length) * 100)}%)`,
);
console.log(
  `Routes with sockets:        ${routedWithSocket.length}/${routeFiles.length} (${Math.round((routedWithSocket.length / routeFiles.length) * 100)}%)`,
);
console.log();
...
console.log("-".repeat(40));
console.log(
  `${"File".padEnd(35)} ${"Notif".padEnd(6)} ${"Email".padEnd(6)} ${"Audit".padEnd(6)} ${"Socket".padEnd(6)}`,
);
console.log("-".repeat(59));
for (const entry of routeFiles) {
  const relPath = relative(join(process.cwd(), "src", "routes"), entry.file);
  console.log(
    `${relPath.padEnd(35)} ${(entry.hasNotifications ? "YES" : "no").padEnd(6)} ${(entry.hasEmails ? "YES" : "no").padEnd(6)} ${(entry.hasAudit ? "YES" : "no").padEnd(6)} ${(entry.hasSocket ? "YES" : "no").padEnd(6)}`,
  );
}
console.log();
...
if (existsSync(notifModelPath)) {
  ...
  uniqueTypes = [...new Set(typeMatches.map((t) => t.replace(/["']/g, "")))].filter((t) =>
    /^[a-z_]+$/.test(t),
  );
  ...
  if (unmappedTypes.length > 0) {
    console.log(
      `Unmapped: ${unmappedTypes.slice(0, 20).join(", ")}${unmappedTypes.length > 20 ? "..." : ""}`,
    );
  }
  console.log();
}
...
const overallScore = Math.round(
  (notifScore * 0.4 + emailScore * 0.2 + auditScore * 0.2 + typeMappingScore * 0.2) * 100,
);
