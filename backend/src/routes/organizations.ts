...
import { getPlanLimits, StorageQuota } from "../lib/db/models/StorageQuota.js";
import { User } from "../lib/db/models/User.js";
import { sendOrganizationInviteEmail } from "../lib/mail/index.js";
import { requireOrgMembership } from "../lib/org-utils.js";
import { isAdminRole, ROLES } from "../lib/rbac/index.js";
...
router.get("/", async (req: AuthRequest, res: Response) => {
  ...
  const orgs = await Organization.find({ _id: { $in: orgIds } })
    .sort({ createdAt: -1 })
    .select("id name slug domain plan businessType industry ownerId logo createdAt updatedAt")
    .lean();
...
router.post("/invite", async (req: AuthRequest, res: Response) => {
  ...
  const membership = await OrgMember.findOne({ orgId: targetOrgId, userId: req.user!.userId })
    .select("role")
    .lean();
...
function resolveOrg(orgId: string) {
  ...
  return Organization.findOne({ $or: filter }).select(
    "_id id name slug domain plan businessType industry ownerId logo gstNumber panNumber cinNumber companyEmail mobileNumber alternateMobileNumber website addressLine1 addressLine2 city state pincode country authorizedPersonName designation authorizedPersonEmail authorizedPersonMobile numberOfEmployees companyDescription createdAt updatedAt",
  );
}
