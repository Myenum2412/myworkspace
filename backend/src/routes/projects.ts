...
router.post("/", upload.single("attachment"), async (req: AuthRequest, res: Response) => {
  ...
  const access =
    req.body.access !== undefined
      ? requireEnum(req.body.access, PROJECT_ACCESS, "access")
      : "Public";
  const status =
    req.body.status !== undefined
      ? requireEnum(req.body.status, PROJECT_STATUSES, "status")
      : "Active";
  ...
  const health = ["on-track", "at-risk", "delayed"].includes(req.body.health)
    ? req.body.health
    : "on-track";
  ...
  if (req.body.deadline) {
    ...
    if (isNaN(d.getTime()))
      throw new AppError(400, "Invalid deadline", { deadline: "must be a valid date" });
    deadline = d;
  }
  ...
  notifyProject
    .created(req.user!.userId, orgId, req.user!.userId, name, project.id)
    .catch(() => {});
...
router.put("/:id", async (req: AuthRequest, res: Response) => {
  ...
  if (existing.orgId !== userOrgId)
    throw new AppError(403, "Not authorized to modify this project");
  ...
  for (const memberId of existingMembers) {
    if (memberId !== actorId) {
      notifyProject
        .updated(memberId, existing.orgId, actorId, projectName, result.id)
        .catch(() => {});
    }
  }
  ...
  if (updates.members !== undefined) {
    const newMembers = (updates.members as string[]).filter(
      (m: string) => !existingMembers.includes(m),
    );
    for (const memberId of newMembers) {
      notifyProject
        .assigned(memberId, existing.orgId, actorId, projectName, result.id)
        .catch(() => {});
    }
  }
...
router.delete("/:id", async (req: AuthRequest, res: Response) => {
  ...
  for (const memberId of (existing.members || []) as string[]) {
    notifyProject
      .deleted(memberId, existing.orgId, req.user!.userId, existing.name, req.params.id)
      .catch(() => {});
  }
