...
function buildFuzzyRegex(query: string): RegExp {
  ...
  const fuzzy = escaped
    .split(/\s+/)
    .map((term) => (term.length > 3 ? term.split("").join(".{0,2}") : term))
    .join(".*");
...
router.get("/", async (req: AuthRequest, res: Response) => {
  ...
  async function searchWithFallback(
    ...
    if (facets.mimeType)
      fullMatch.mimeType = { $regex: `^${escapeRegex(facetMimeType!)}`, $options: "i" };
    ...
    try {
      ...
      return {
        results: results
          .slice(0, limit)
          .map((r: any) => ({ ...r, _searchType: entityType, _weight: weight })),
        hasMore,
      };
    } catch {
  ...
  const [
    ...
    searchWithFallback(
      ...
      "id orgId name originalName mimeType size uploaderId createdAt",
      "files",
      10,
    ),
    searchWithFallback(
      Folder,
      { orgId, deletedAt: null },
      "id name path parentId createdAt",
      "folders",
      5,
    ),
    searchWithFallback(
      Task,
      { orgId },
      "id orgId title description status priority createdAt",
      "tasks",
      7,
    ),
  ...
  if (userIds.length > 0) {
    const uploaders = await User.find({ _id: { $in: userIds } })
      .select("_id name")
      .lean();
  ...
  const hasMore =
    ...
    employeesResult.hasMore ||
    clientsResult.hasMore ||
    teamsResult.hasMore;
  ...
  const response = {
    ...
    data: {
      ...
      folders: foldersResult.results.map((f: any) => ({
        id: f.id,
        name: f.name,
        path: f.path,
        parentId: f.parentId,
      ...
      projects: projectsResult.results.map((p: any) => ({
        id: p.id,
        name: p.name,
        description: p.description,
        status: p.status,
      })),
      employees: employeesResult.results.map((e: any) => ({
        id: e.id,
        name: e.name,
        email: e.email,
        role: e.role,
      })),
      clients: clientsResult.results.map((c: any) => ({
        id: c.id,
        name: c.name,
        email: c.email,
        company: c.company,
      })),
      teams: teamsResult.results.map((t: any) => ({
        id: t.id,
        name: t.name,
        description: t.description,
      })),
...
router.get("/autocomplete", async (req: AuthRequest, res: Response) => {
  ...
  const searchNameOrTitle = (
    ...
    return model
      .find({ ...match, [field]: { $regex: autocompleteRegex } })
      .limit(limit)
      .select(`${field} id`)
      .lean();
  };
