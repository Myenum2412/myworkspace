...
export async function createIndexes(): Promise<void> {
  ...
  // Org Members
  await c("org_members").createIndex(
    { userId: 1, orgId: 1 },
    { unique: true, name: "idx_orgmembers_user_org" },
  );
  ...
  await c("tasks").createIndex(
    { orgId: 1, status: 1, assigneeId: 1, createdAt: -1 },
    { name: "idx_tasks_org_status_assignee_created" },
  );
  ...
  // Team Members
  await c("teammembers").createIndex(
    { orgId: 1, teamId: 1, userId: 1 },
    { unique: true, name: "idx_teammembers_org_team_user" },
  );
  await c("teammembers").createIndex({ orgId: 1, userId: 1 }, { name: "idx_teammembers_org_user" });
  await c("teammembers").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_teammembers_org_createdby" },
  );

  // Time Entries
  await c("time_entries").createIndex({ orgId: 1, date: -1 }, { name: "idx_timeentries_org_date" });
  await c("time_entries").createIndex(
    { orgId: 1, userId: 1, date: -1 },
    { name: "idx_timeentries_org_user_date" },
  );
  await c("time_entries").createIndex(
    { orgId: 1, status: 1 },
    { name: "idx_timeentries_org_status" },
  );
  await c("time_entries").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_timeentries_org_createdby" },
  );

  // Upload Sessions
  await c("uploadsessions").createIndex(
    { updatedAt: 1 },
    { expireAfterSeconds: 7 * 24 * 60 * 60, name: "idx_uploadsessions_ttl" },
  );

  // Activity Logs
  await c("activitylogs").createIndex(
    { orgId: 1, createdAt: -1 },
    { name: "idx_activity_org_created" },
  );
  await c("activitylogs").createIndex({ orgId: 1, userId: 1 }, { name: "idx_activity_org_user" });
  await c("activitylogs").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_activity_org_createdby" },
  );
  await c("activitylogs").createIndex(
  ...
  await c("activitylogs").createIndex(
    { createdAt: 1 },
    { expireAfterSeconds: 365 * 24 * 60 * 60, name: "idx_activitylogs_ttl" },
  );

  // Sessions
  await c("sessions").createIndex(
    { userId: 1, loginTime: -1 },
    { name: "idx_sessions_user_login" },
  );
  ...
  // Notifications
  await c("notifications").createIndex(
  ...
  await c("notifications").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_notifications_org_createdby" },
  );
  await c("notifications").createIndex(
    { createdAt: 1 },
    { expireAfterSeconds: 90 * 24 * 60 * 60, name: "idx_notifications_ttl" },
  );
  ...
  await c("files").createIndex(
    { checksum: 1, orgId: 1, folderId: 1 },
    {
      unique: true,
      partialFilterExpression: { deletedAt: null },
      name: "idx_files_checksum_org_folder_dedup",
    },
  );
  await c("files").createIndex({ id: 1 }, { unique: true, name: "idx_files_id" });

  // File Versions
  await c("fileversions").createIndex(
    { orgId: 1, fileId: 1, versionNumber: -1 },
    { name: "idx_fileversions_org_file_version" },
  );
  await c("fileversions").createIndex(
    { orgId: 1, uploadedBy: 1 },
    { name: "idx_fileversions_org_uploadedby" },
  );
  ...
  await c("folders").createIndex(
    { orgId: 1, clientId: 1, path: 1 },
    { unique: true, name: "idx_folders_org_client_path" },
  );
  ...
  await c("sharelinks").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_sharelinks_org_createdby" },
  );
  ...
  await c("fileshares").createIndex(
    { orgId: 1, sharedByUserId: 1 },
    { name: "idx_fileshares_org_sharer" },
  );
  await c("fileshares").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_fileshares_org_createdby" },
  );
  await c("fileshares").createIndex({ id: 1 }, { unique: true, name: "idx_fileshares_id" });

  // Projects
  await c("projects").createIndex(
    { orgId: 1, createdAt: -1 },
    { name: "idx_projects_org_created" },
  );
  await c("projects").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_projects_org_createdby" },
  );
  ...
  // Client Workspaces
  await c("clientworkspaces").createIndex(
    { orgId: 1, clientId: 1 },
    { unique: true, name: "idx_clientworkspaces_org_client" },
  );
  await c("clientworkspaces").createIndex(
    { id: 1 },
    { unique: true, name: "idx_clientworkspaces_id" },
  );
  ...
  await c("clientusers").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_clientusers_org_createdby" },
  );
  ...
  // Client Audit Logs
  await c("clientauditlogs").createIndex(
    { orgId: 1, createdAt: -1 },
    { name: "idx_clientaudit_org_created" },
  );
  await c("clientauditlogs").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_clientaudit_org_createdby" },
  );
  ...
  // Messages
  await c("messages").createIndex(
    { orgId: 1, createdAt: -1 },
    { name: "idx_messages_org_created" },
  );
  await c("messages").createIndex({ orgId: 1, senderId: 1 }, { name: "idx_messages_org_sender" });
  await c("messages").createIndex(
    { orgId: 1, createdBy: 1 },
    { name: "idx_messages_org_createdby" },
  );
