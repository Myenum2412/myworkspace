...
export function registerAllHandlers(): void {
  ...
  jobRegistry.register("system_maintenance", async (payload, jobId, orgId, userId) => {
    ...
    if (!orgId) {
      logger.debug({ jobId, type: "system_maintenance" }, "System maintenance job executed (no org scope)");
      return;
    }
    ...
    const sessions = await Session.find({ orgId, loginTime: { $gte: dayStart, $lt: dayEnd } }).lean();
    ...
    logger.info(
      {
        ...
        byCategory,
        topEvents: Object.entries(byName).sort((a, b) => b[1] - a[1]).slice(0, 10),
      },
  ...
  jobRegistry.register("log_rotation", async () => {
    ...
    const purge = async (
      ...
      logger.info({ collection: label, deleted: res.deletedCount || 0 }, "Log rotation purged collection");
    };
