...
export const notifyPermission = {
  ...
  async clientPortalAccessGranted(
    userId: string,
    orgId: string,
    grantedBy: string,
    clientName: string,
  ...
  async suspiciousPermissionChange(userId: string, orgId: string, details: string) {
    return createNotification({
      ...
      orgId,
      createdBy: userId,
      type: "suspicious_permission_change",
      category: "permissions",
      priority: "critical",
      ...
      link: "/admin/security",
      actions: [
        { label: "Review Security", action: "view", url: "/admin/security", primary: true },
      ],
      metadata: { details },
    });
  },
