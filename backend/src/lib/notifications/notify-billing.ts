...
export const notifyBilling = {
  ...
  async renewalReminder(
    ...
    return createNotification({
      ...
      link: link || "/billing",
      actions: [
        { label: "Manage Subscription", action: "view", url: link || "/billing", primary: true },
      ],
  ...
  async storageUpgradeAvailable(userId: string, orgId: string, link?: string) {
    return createNotification({
      ...
      orgId,
      createdBy: userId,
      type: "storage_upgrade_available",
      category: "billing",
      ...
      actions: [
        { label: "View Plans", action: "view", url: link || "/settings/billing", primary: true },
      ],
    });
  },
  ...
  async planLimitReached(userId: string, orgId: string, limitType: string, link?: string) {
    return createNotification({
      ...
      link: link || "/settings/billing",
      actions: [
        { label: "Upgrade Plan", action: "view", url: link || "/settings/billing", primary: true },
      ],
      metadata: { limitType },
    });
  },
  ...
  async additionalUsersPurchased(userId: string, orgId: string, count: number, link?: string) {
    return createNotification({
      ...
      orgId,
      createdBy: userId,
      type: "additional_users_purchased",
      category: "billing",
