...
export const notifySecurity = {
  ...
  async emailChanged(userId: string, orgId: string, oldEmail: string, newEmail: string) {
    return createNotification({
      ...
      actions: [
        { label: "Review Changes", action: "view", url: "/settings/security", primary: true },
      ],
  ...
  async apiAbuseDetected(userId: string, orgId: string, endpoint: string, reason: string) {
    return createNotification({
      ...
      actions: [
        { label: "Review API Keys", action: "view", url: "/settings/developer", primary: true },
      ],
  ...
  async unauthorizedAccessAttempt(
    ...
    return createNotification({
      ...
      actions: [
        { label: "Review Security Logs", action: "view", url: "/admin/security", primary: true },
      ],
