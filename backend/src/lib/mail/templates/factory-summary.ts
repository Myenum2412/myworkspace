import type { EmailData, SummaryItem } from "./types.js";
...
export const buildDailyDigest = (
  ...
  ...(pendingApprovals.length > 0
    ? {
        warning:
          `You have ${pendingApprovals.length} pending approval${pendingApprovals.length > 1 ? "s" : ""} that need your review.` as const,
      }
    : {}),
...
export const buildWeeklyDigest = (
  ...
  intro: [
    "Here is your weekly activity summary. See what happened across your workspace this week.",
  ],
  details: [
...
export const buildUnreadNotificationsReminder = (
  ...
  intro: [
    `You have ${unreadCount} unread notification${unreadCount > 1 ? "s" : ""} that ${unreadCount > 1 ? "are" : "is"} waiting for your attention.`,
  ],
