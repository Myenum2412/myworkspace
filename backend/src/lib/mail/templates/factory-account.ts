import type { EmailCard, EmailData } from "./types.js";
...
export const buildWelcomeEmail = (
  ...
  return {
    ...
    ...(tempPassword
      ? {
          securityNotice: true as const,
          warning: "For security, change your temporary password after your first login." as const,
        }
      : {}),
    providerIcon,
    socialLinks: {
...
export const buildVerificationEmail = (firstName: string, verificationUrl: string): EmailData => ({
...
export const buildVerifiedEmail = (firstName: string, loginUrl: string): EmailData => ({
  ...
  intro: [
    "Your email address has been successfully verified. You can now access all features of your Workspace.",
  ],
...
// 5. Team Member Added
export const buildTeamMemberAdded = (
  firstName: string,
  teamName: string,
  loginUrl: string,
): EmailData => ({
  ...
  intro: [
    `You have been added to the ${teamName} team. You can now view and collaborate on team projects.`,
  ],
...
export const buildFirstLogin = (firstName: string): EmailData => ({
  ...
  intro: [
    "We noticed you just logged in for the first time. Welcome aboard!",
    "Here are a few tips to get the most out of your workspace.",
  ],
...
export const buildGettingStarted = (firstName: string, docsUrl: string): EmailData => ({
  ...
  intro: [
    "Ready to supercharge your productivity? Check out our quick start guide to learn the basics.",
  ],
...
export const buildProfileReminder = (firstName: string, profileUrl: string): EmailData => ({
  ...
  intro: [
    "We noticed your profile is missing a few details. Complete your profile to help your team know you better.",
  ],
...
export const buildWorkspaceSetupComplete = (
  workspaceName: string,
  loginUrl: string,
): EmailData => ({
  ...
  intro: [
    `Your workspace "${workspaceName}" has been successfully configured and is ready to use.`,
  ],
...
export const buildNewDeviceLogin = (
  firstName: string,
  deviceName: string,
  location: string,
): EmailData => ({
  ...
  intro: ["We noticed a new login to your account from an unrecognized device."],
  details: [
...
export const buildSubscriptionActivated = (
  firstName: string,
  planName: string,
  loginUrl: string,
): EmailData => ({
...
// 14. Trial Started
export const buildTrialStarted = (
  firstName: string,
  daysLeft: number,
  loginUrl: string,
): EmailData => ({
...
export const buildTrialEndingSoon = (
  firstName: string,
  daysLeft: number,
  upgradeUrl: string,
): EmailData => ({
  ...
  intro: [
    "Your free trial is almost over. Upgrade your plan to keep access to all premium features.",
  ],
...
export const buildRenewalConfirmation = (firstName: string, amount: string): EmailData => ({
  ...
  intro: [
    `Your subscription has been successfully renewed for ${amount}. Thank you for your continued support!`,
  ],
  supportEmail: "support@workspace.com",
});
...
export const buildAccountDeactivated = (firstName: string): EmailData => ({
  ...
  intro: [
    "Your Workspace account has been deactivated by an administrator. If you believe this is a mistake, please contact support.",
  ],
  supportEmail: "support@workspace.com",
});
...
export const buildAccountReactivated = (firstName: string, loginUrl: string): EmailData => ({
  ...
  intro: [
    "Good news! Your Workspace account has been reactivated. You can now log in and access your data.",
  ],
  button: { text: "Log In", url: loginUrl },
  supportEmail: "support@workspace.com",
});
...
export const buildPasswordDeliveredEmail = (
  name: string,
  email: string,
  password: string,
  loginUrl: string,
