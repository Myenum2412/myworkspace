...
export class BusinessIntelligenceEngine {
    ...
    const templates: Record<string, DashboardWidget[]> = {
      ...
      storage: [
        ...
        },
        {
          ...
          type: "table",
          title: "Largest Files",
          metric: "storage.largest",
          aggregation: "sum",
          filters: {},
          period: "year",
          visualization: "table",
          size: "full",
          config: { maxRows: 20, sortBy: "size" },
        },
      ],
    };
  ...
  async getWidgetData(
    ...
    switch (widget.metric) {
      ...
      case "activity.recent": {
        const logs = await ActivityLog.find({ orgId }).sort({ createdAt: -1 }).limit(10).lean();
  ...
  async getOrgHealthScore(orgId: string): Promise<{
    ...
    const engagementRate =
      users.length > 0 ? users.filter((u) => u.status === "online").length / users.length : 0;

    const productivity = Math.round(
      Math.min(
        100,
        (taskCompletionRate * 60 + Math.max(0, 1 - overdueTasks / Math.max(1, tasks.length)) * 40) *
          100,
      ),
    );
    ...
    const storageUtil =
      files.length > 0
        ? Math.round(
            Math.min(
              100,
              (files.reduce((s, f) => s + (f.size || 0), 0) / (10 * 1024 * 1024 * 1024)) * 100,
            ),
          )
        : 0;
    ...
    const categories = {
      ...
      engagement,
      growth: growthScore,
      storage: Math.max(0, 100 - storageUtil),
    };
    ...
    return {
      ...
      breakdown: {
        ...
        overdueTasks: {
          score: Math.max(0, 100 - overdueTasks * 5),
          max: 100,
          label: "Overdue Tasks",
        },
