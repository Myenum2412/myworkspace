...
const installationSchema = new Schema<IInstallation>(
  {
    ...
    status: {
      type: String,
      enum: ["installing", "active", "error", "disabled", "uninstalled"],
      default: "installing",
    },
    version: { type: String, default: "1.0.0" },
    lastHealthCheckAt: Date,
...
export const MarketplaceIntegration = model<IMarketplaceIntegration>(
...
export class MarketplaceEngine {
  async registerIntegration(params: {
    ...
    provider: string;
    description?: string;
    category: IMarketplaceIntegration["category"];
    configSchema?: Record<string, unknown>;
    permissions?: string[];
    pricing?: string;
  ...
  async getOrgInstallations(orgId: string): Promise<IInstallation[]> {
    return Installation.find({ orgId, status: { $ne: "uninstalled" } })
      .sort({ installedAt: -1 })
      .lean() as any;
  }
  ...
  async getMarketplaceStats(): Promise<{
    ...
    const top = await MarketplaceIntegration.find({ isPublished: true })
      .sort({ installCount: -1 })
      .limit(10)
      .lean();
    return {
