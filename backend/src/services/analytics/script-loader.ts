...
const KNOWN_SCRIPTS: ThirdPartyScript[] = [
  ...
  },
  {
    ...
    name: "Amplitude",
    category: "analytics",
    src: "https://cdn.amplitude.com/libs/analytics-browser-",
    async: true,
    defer: true,
    consentRequired: true,
  },
  {
    ...
    name: "Mixpanel",
    category: "analytics",
    src: "https://cdn.mxpnl.com/libs/mixpanel-",
    async: true,
    defer: true,
    consentRequired: true,
  },
...
export class ScriptLoaderService {
  ...
  generateScriptTag(script: ThirdPartyScript, apiKey?: string): string {
    let src = script.src || "";
    if (apiKey && script.id === "ga4")
      src = `https://www.googletagmanager.com/gtag/js?id=${apiKey}`;
  ...
  generateConsentAwareLoader(scripts: ThirdPartyScript[], consentEndpoint: string): string {
    const scriptDefs = JSON.stringify(
      scripts.map((s) => ({
        id: s.id,
        name: s.name,
        category: s.category,
