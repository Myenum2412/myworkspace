...
import { MessageIngestor, type IngestedMessage } from "../pipeline/message-ingestor.js";
...
export class AIAgent {
  ...
  async run(request: AgentRequest): Promise<AgentResponse> {
    ...
    try {
      ...
      const { session, messages, memories } = await this.memoryManager.loadContextForSession(
      ...
      while (toolIterationCount < this.config.maxToolIterations) {
        ...
        let intent = ingested.intent.intent;
