    ...
    it("concurrent updates to same task do not drop fields", async () => {
      ...
      const [r1, r2] = await Promise.all([
        request(server)
          ...
          , headers:{ctx.headers}
          , payload:{ title: "Updated Title", orgId: ctx.orgId },
        request(server)
          .patch(`/api/tasks/${taskId}`)
          , headers:{ctx.headers}
          , payload:{ priority: "urgent", orgId: ctx.orgId },
      ]);
  ...
  describe("login lockout race", () => {
    it("concurrent failed logins near threshold are counted correctly", async () => {
      ...
      await request(server)
        ...
        , payload:{ name: "Lockout Test", email, password: "SecurePass123" };

      // Fire 6 concurrent failed login attempts (threshold is 5)
      const attempts = Array.from({ length: 6 }, () =>
        request(server).post("/api/auth/login"), payload:{ email, password: "wrongpassword" },
      );
  ...
  describe("parallel document creation with same unique field", () => {
    it("duplicate email signup returns 409", async () => {
      ...
      const [r1, r2] = await Promise.all([
        request(server)
          .post("/api/auth/signup")
          , payload:{ name: "User1", email, password: "SecurePass123" },
        request(server)
          .post("/api/auth/signup")
          , payload:{ name: "User2", email, password: "SecurePass123" },
      ]);
