...
describe("tasks CRUD (with Mongo)", () => {
  it("POST /api/tasks persists a task in caller's org", async () => {
    ...
    const res = await agent()
      .post("/api/tasks")
      .set(headers)
      .send({ orgId, title: "Hello task", priority: "high" });
  ...
  it("GET /api/tasks returns only caller's org rows", async () => {
    ...
    await agent()
      .post("/api/tasks")
      .set(headers)
      .send({ orgId, title: "My task", priority: "medium" });
    await agent()
      .post("/api/tasks")
      .set(other.headers)
      .send({ orgId: other.orgId, title: "Their task", priority: "medium" });
  ...
  it("DELETE /api/tasks removes the row", async () => {
    ...
    const created = await agent()
      .post("/api/tasks")
      .set(headers)
      .send({ orgId, title: "Removable", priority: "medium" });
    const id = created.body.data.taskId;
