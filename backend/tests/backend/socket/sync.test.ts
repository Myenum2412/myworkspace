...
beforeEach(async () => {
  ...
  io.on("connection", (s) => {
    s.join(`org:${(s as any).orgId}`);
    s.join(`user:${(s as any).userId}`);
  });
...
describe("Socket delta contract (org-scoped broadcast)", () => {
  it("broadcasts an event only to clients in the same org room", async () => {
    ...
    const b1Got = new Promise((resolve, reject) => {
      ...
      b1.once("task:created", (d) => {
        clearTimeout(t);
        reject(new Error(`B should NOT have received it, got: ${JSON.stringify(d)}`));
      });
    });

    // Server-side emit mimicking what route handlers do via socketIOManager.emitToOrg
    io.to("org:orgA").emit("task:created", {
      id: "t1",
      orgId: "orgA",
      title: "Hello",
      status: "todo",
    ...
    const a2Got = await new Promise((r) => {
      a2.once("task:created", r);
