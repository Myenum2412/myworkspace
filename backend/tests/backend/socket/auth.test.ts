...
describe("Socket auth & handshake", () => {
  it("rejects a client with no token", async () => {
    await expect(rawConnect(server.url, "")).rejects.toThrow();
  });

  it("rejects a client with an invalid token", async () => {
    await expect(rawConnect(server.url, "not-a-real.jwt.token")).rejects.toThrow();
  });
