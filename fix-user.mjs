...
console.log(
  "Current user:",
  JSON.stringify({ _id: user._id, email: user.email, orgId: user.orgId, name: user.name }),
);

const org = await db
  .collection("organizations")
  .findOne({ _id: new ObjectId("6a37d3bd1032bb9fb9ed2251") });
...
console.log(
  "OrgMember records:",
  members.length,
  members.map((m) => ({ orgId: m.orgId, userId: m.userId, role: m.role })),
);

if (!user.orgId) {
  await db
    .collection("users")
    .updateOne({ _id: user._id }, { $set: { orgId: "6a37d3bd1032bb9fb9ed2251" } });
